  ...
  return (
    ...
    >
      <div className="flex mt-5">
        <p className={aboutClassName}>{t('own passion project')}</p>
      </div>
      <div className="flex justify-between items-center gap-3 mt-3">
        <p className={aboutClassName}>
          ...
          {t('if you wish')} {t('do my best')}
        </p>
        <img
          className="w-9 h-9 wiggle cursor-pointer"
          src={coffeeLogo}
          title={t('Buy me a coffee?')}
          alt={t('Buy me a coffee?')}
          onClick={() => window.open(t('KOFI_LINK'), '_blank')}
        />
      </div>
      <div className="flex mt-3">
        <p className={aboutClassName}>{t('much love to')}</p>
      </div>
      <div className="flex mt-3">
        <textarea
          className="local-font text-xs text-gray-400 dark:text-gray-500 w-full border-solid border-2 rounded bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-600"
          rows={3}
